...
Now that we have considered how to simplify propositions, we wish to use
this technology to simplify programming problems. Suppose we have a goal
of type $\Delta \rightarrow T$, where $\Delta$ is some telescope of
hypotheses. There are various things we can do to simplify the problem,
such as:

-   splitting up $\Sigma$-types into their components;
...
We say simplification is *at the top level* if we are simplifying
...
Functions from the unit type are simply constants, so we simplify them
as such.
...
Given a function from a $\Sigma$-type, we can split it into its
components.
...
Otherwise, we simplify $\Pi$-types by introducing a hypothesis, provided
...
To simplify a $\Pi$-type when not at the top level, we have to create a
subgoal.
...
When the goal is a proof of a proposition, and we are at the top level,
we can just invoke propositional simplification...
