...
finely confected structures, subtly designed, so subtly different. The
...
library~\cite{nisse:asl} sports a writhing mass of list-like
structures, including vectors, bounded-length lists, difference lists,
reflexive-transitive closures---the list is petrifying. In this paper,
we equip ourselves to fight this gorgon, with \emph{reflection}.
...
The business of belonging to a datatype is itself a relative notion,
depending on the type's \emph{declaration}. Most typed functional
languages feature a datatype declaration construct, external to
and extending, the language for defining values and programs.
