...
of descriptions. In and of itself, the codes, $\Desc_n$, is nothing
but a datatype in $\Set_{n+1}$. We are in the same situation as with
$\EnumU$: we ought to be able to describe the codes of $\Desc_n$ in
$\Desc_{n+1}}$ itself. Hence, this code would be a first-class
...
%
\[
\DSigma{(\Bhab{\V{S}}{\Set_n})}{(\Bhab{\V{D}}{\V{S} \To \Desc_n})} : \Desc_n\\
\]
%
...
{\stkm{ \DUnit  \\
        \DSigma{\Set_n}{\LAM{\V{S}} \DHindx{\V{S}}{\DUnit}}   \\
        \DIndx{\DUnit}                                    \\
        \DSigma{\Set_n}{\LAM{\_} \DIndx{\DUnit}}
}}
}\]
%
...
codes of the universe of descriptions. Taking the fixpoint of \(\descop{\DescD_n}\)
gives us a datatype exactly like $\Desc_n$. Might we be
so bold as to take \(\Desc_n \mapsto \Mu{\DescD_n}\) as the levitating
...
The definition \(\Desc_n \mapsto \Mu{\DescD_n}\) is circular,
...
\Desc_n \mapsto
    \Mu{\stk{(
        \DSigma{\EnumT{\sqr{ \DUnit\:\: \SYMBDSigma\:\: \SYMBDIndx\:\: \SYMBDHindx}}\\}
               {\;\SYMBswitch\:\sqr{ \DUnit\:\: \SYMBDSigma\:\: \SYMBDIndx\:\: \SYMBDHindx}\:
                                   (\LAM{\_}\Desc)\\
                             \;\sqr{\stkm{
                                   \DUnit \\
                                   \DSigma{\Set_n}{\LAM{\V{S}} \DHindx{\V{S}}{\DUnit}} \\
