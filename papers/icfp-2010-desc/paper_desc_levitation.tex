...
type formers ($\EnumU$) and constructors ($\NilE$ and $\ConsE$) are now
...
the data-types we have seen so far, $\DescD$ first consists in choosing
...
packing the $H$ in $\Set$ with a recursive argument.
...
The factorised universe of description is presented in
...
translated into this new presentation. \note{We might want to re-do
  the Nat example here, as we encode once again in $\IDesc$} In the
...
\note{Update to the factorised presentation}
...
\note{ Triple check. That doesn't work, right? }
