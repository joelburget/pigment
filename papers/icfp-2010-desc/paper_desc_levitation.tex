...
Let us examine the consequences. First of all, we discover that the
...
the system of codes, $\Desc$, is nothing but a data-type. We are in
the same situation than with $\EnumU$: we ought to be able to describe
...
fully make sense: these were actually the tags we are
defining. \note{This sentence badly needs a rephrasing.}
...
description. This gives the developer the ability, for any data-type,
...
programming style is reminiscent to the type-indexed data
types~\cite{hinze:generic-haskell}, as implemented in Generic Haskell:
from data-types, we built new data-types and equip them with some
structure.
...
it. Extending a data-type might give rise to a more structured object,
as was the case here.  So, secondly, we have equipped this new
data-type with its inherent structure: the \bind\ and
