...
This language has values, conditional
...
We express our syntax as a tagged indexed description, indexing
over obhect language types $\Ty$. We note
that some constructors are always available, namely $\SYMBEval$ and
$\SYMBEcond$. On the other hand, $\SYMBEplus$ and $\SYMBEle$
constructors are index-dependent, with $\SYMBEplus$ available just when
building a $\Enat$, $\SYMBEle$ just for $\Ebool$. The code,
%%%in Figure~\ref{fig:hexpr-full},
below,
reflects this intuition, with the first
...
Given the syntax, let us supply the
semantics. We implement an evaluator as a catamorphism:
%
\[\stk{
\evalH : \PI{\V{ty}}{\Ty} 
...
of~\citet{augustsson.carlsson:dependent.interpreter}, with
help from the generic catamorphism.
