...
we are already quoting a standard type-former. In the alternative
presentation, we go further and entirely present our codes as
quotation of standard type-formers. This presentation is shown in
Figure~\ref{fig:type-former-desc}.
...
addition and comparison operator in $\Nat$, respectively name
...
algebra -- is sufficient, as $\F{cataI}$ gives, for free, the
full evaluator. The implementation is as follow:
...
\note{We have not defined \F{cataI}!}
...
\emph{closed} terms. By abstracting over $\Var{}$, it is possible to
...
-- contains values. A variable is therefore a $\Eval{}$ that contains
a pointer to a particular element of the finite set -- an element of
$\EnumT{dom}$. The extra argument to $\Var{}{}$ is the domain of this
context.
...
proceed in two steps: first, replace the variables by their value in
the context; then, evaluate the resulting closed term. Thanks to
...
than what we are currently provided. In particular, this is
reminiscent to a free monad, where $\Eval{}$ is the ``return''
introducing variables. Instead of implementing $\substH$ in this
special case, we shall implement it in a generic setting, and express
our example in that setting.
...
datatype $\HExprD$ and $\HExprVarD$ enjoys a monadic structure. We had
...
corresponds to the closed expression, $\HExprD$. Similarly,
$$\FreeIMonad{\HExprFreeD}{\Ty}{(\LAM{ty} \Val{ty} + \Var{ty})}$$
...
terms in $\FreeIMonad{\HExprFreeD}{\Ty}{\Val{}}$ to values in
