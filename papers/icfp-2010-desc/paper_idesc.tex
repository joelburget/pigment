...
a more compositional algebra of codes, where $\Desc$ is rather
biased towards the familiar right-nested tuples. We now have
\(\DVar{i}\) for recursive `subrequests' at a chosen index \(i\),
with tupling handled by \(\DProd{}{}\) and higher-order branching
...
In order to gain some intuition of $\SYMBIDesc$, let us revisit the
natural numbers:
%
\[\stk{
\NatD : \IDesc{\Unit} \\
...
Of course, we can \(\upgrade\) any inductive type.
...
However, sometimes, we can actually remove these equations altogether. Let us
...
Note that forcing a description is not guaranteed to remove all
...
descriptions. For brevity, we shall also overload $\IMu{ty}{D}$ for
$\IMu{ty}{(\toIDesc{D})}$, as the bidirectional presentation let us
...
operations. A summary of the components defining descriptions and
their indexed counterparts is presented in
Table~\ref{tab:sumup-operators}.
...
\hline
Desc   & IDesc & Role & Status \\
\hline
\hline
\multicolumn{2}{|c|}{$\EnumU$}                  & Build finite sets           & Levitated \\
\hline
$\Desc$               & $\IDesc$               & Describe functors   & Levitated \\
\hline
$\SYMBdescop{\_}$         & $\SYMBidescop{\_}{\_}$         & Interpret descriptions      & Hardwired \\
\hline
$\SYMBMu$, $\SYMBCon$ & $\SYMBIMu_I$, $\SYMBCon$ & Define fixpoints & Hardwired \\
\hline
$\SYMBind$, $\SYMBAll$, $\SYMBall$ & $\inductionI$, $\SYMBAllI$, $\SYMBallI$ & Induction principle & Hardwired \\
\hline
\end{tabular}
