...
\note{pwm: The exception being non dependent product -- we might want to
explain why this is.} 
...
that $\alpha$ can be any type. \note{Polymorph? Will any ghost try to
  ...
  Aaaaaaaaaaargh\ldots (Wha? - pwm)}
\note{pwm: I don't really get the need for this $\Epolym$ notation. Why not
just abstract over an elem of $\Ty£$?} 
...
\note{the big square brackets should be red (pwm: use \\bigRedBracket :)) }
...
\note{pwm: $\evalOne$ rather illustrates my point about the 2 types of IFunc
floating around, $\idescop{\_}{}{}$ is defined using the 1st notion, and used it
here as if it were the 2nd}
...
\substH  : & \PITEL{\V{\domNat}}{\EnumU} 
          ...
          &  \begin{array}{@{}ll}
             \PI{\V{\sigma}}{& \PITEL{\V{\domNat}}{\EnumU} \\
                             ...
                             & \PI{\V{ty}}{\Ty} (\Val{ty} + \Var{dom_{ty}}{ty}) \To
                               \IMu{\Ty}{(\toIDesc{\HExprD})}{ty}}
             \end{array}\\
          & \PI{\V{ty}}{\Ty}
