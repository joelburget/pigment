...
This is a very inefficent traversal strategy: after we regive a girl, we look at
all her children (including those generated by regive). We should replace |nextStep|
and process the children first.

> gimme :: ProofState ()
> gimme = do
>     much goOut
>     much (nextStep >> (regive `catchError` (\x -> error ("fail: " ++ show x))))
>     much goOut
>     s <- prettyProofState
>     case trace s True of
>       True -> return ()
>   where
>     regive :: ProofState ()
>     regive = do
