...
In Section~\ref{sec:developments}, we have developed the notion of
|Development|, a tree reifing the proof construction process. In order
to navigate this tree, we have computed its zipper in
Section~\ref{sec:proof_context}, the |ProofContext|. At this stage, we
have a notion of \emph{movement} in the proof context.
...
(Section~\ref{sec:proof_state_monad}). Indeed, when moving down, we
might hit a news bulletin. A news bulletin is a lazy edition
...
in Figure~\ref{fig:navigation-devpmt}: we have that the development
|B| is in focus, with |y| above the cursor and |z| below it.
...
> putEnterCurrent (EDEF ref xn dkind dev ty) = do
>     l <- getLayer
...
>                   propagateNews True visitedNews es'
>                   return ()
>                 Right param -> do
>                   -- Parameter:
>
