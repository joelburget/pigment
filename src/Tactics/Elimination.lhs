...
form $$\Gamma, \Delta \vdash ? : T$$ where $\Gamma$ is the list of
external hypotheses, $\Delta$ is the list of internal hypotheses and $T$
...
to the first term of $\Delta$ in the context. If the user provides no
reference, we will assume that $\Gamma$ is empty, so the hypotheses are
all internal.

Obviously, we also need to be provided the eliminator we want to use. We
expect the user to provide the eliminator for us in the form
$$\Gamma, \Delta \vdash e : (P : \Xi \rightarrow \Set)
                            \rightarrow \vec{m}
                            \rightarrow P \vec{t}$$ where we call $P$
...
we need to check that it *is* an eliminator, that is:
...
-   the type $\Xi \rightarrow \Set$ of the motive, and
...
danger) of manually renaming those references.
...
We will build the motive *in-place*, instead of analyzing the eliminator
...
name of the rebuilt eliminator, the motive type and the list of targets;
the cursor is left on the motive subgoal.
...
Convert the module to a goal, solve it (using the subgoals created
above) and go to the next subproblem, i.e. making the motive $P$:
...
iterate that process, up to the point where all the $\Pi$s have been
consummed.
...
question, the best is still to read the Sanctified Papers
...
Second try: learning from the targets.
...
the motive in another context than the fixed context $\Delta$. We can
get this by abstracting $\Delta$ in $P$, without forgetting to bring
$\vec{t}$ and $T$ under this new context:

$$P \equiv \lambda (\Xi) . \Pi (\Delta') .
     (\Xi) == \vec{t}\ [\Delta'/\Delta] \Rightarrow T[\Delta'/\Delta]$$

Fourth try: being less parametric.

However, there is a slight problem here. The motive and the methods are
very likely to be parameterized over $\Delta$. If we use the motive
above, we are asking for trouble in the definition of the methods.
Remember that a method typically looks like:
...
parameter (because it *is* just a parameter).

So, we need to chunk $\Delta$ in two contexts:

-   $\Delta_0$, containing terms involved in the type of the methods and
    motive, and their respective dependencies; and

-   $\Delta_1$, containing the free terms
...
Fifth try: simplifying equations.
...
goal. Having replaced $t_i$, we can remove the corresponding abstraction
in the telescope $\Pi (\Delta_1')$.

As one would expect, this process must be carried from the left to the
right, as equations might simplify further down the line. However, there
is a subtlety in that, when discovering a simplification, one must also
rename the previously discovered constraints, as they might mention the
simplified term.
...
We have the internal context $\Delta$ lying around. We also have access
to the type of the eliminator, that is the type of the motive and the
methods. Therefore, we can already split $\Delta$ into its parametric
...
these types, making them useless.

The real solution is to go back to the type of the eliminator. We unfold
it with fresh references. Doing so, we are ensured that there is no
pollution, and we get what we asked for.
...
Extracting an element of $\Delta_1$
...
When renaming a forwards list of binders, we need to update the types of
the corresponding references and update those references in later
binders. Note that we never update the argument term (the second
component of the binder) as it needs to remain in the scope of the
original context.
...
hypotheses. Moreover, we have turned $\Delta_1$ into a list of binders.
...
We will be renaming references when we solve constraints, but we need to
...
when simplifying the motive (e.g. splitting sigmas).
...
pair. We make a new subgoal by currying the motive type, solve the
current motive with the curried version, then continue with the target
replaced by its projections. We exclude the case where the target is a
variable, because if so we might be able to simplify its constraint.
...
To the left, we have a backwards list of binders: updated references and
types, and non-updated argument terms.

To the right, we have a forwards list of constraints: references in
$\Xi$ together with the term representation of their type, and typed
terms in $\Delta$ to which the references are equated.

> simplifyMotive :: Bwd Binder -> Fwd Constraint -> INTM
>     -> ProofState (Bwd Binder, INTM)

For each constraint, we check if the term on the right is a reference.
If so, we check the equation is homogeneous (so substitution is allowed)
and look for the reference in $\Delta$. If we find it, we can simplify
by removing the equation, updating the binder and renaming the following
...
To pass a constraint, we append a binder with a fresh reference whose
type is the proof of the equation. When applying the motive, we will
need to use reflexivity applied to the non-updated target.
...
Extract non-parametric, non-removable hypotheses $\Delta_1$ from the
context $\Delta$:
...
Introduce $\Xi$ and generate constraints between its references and the
targets:

>     (constraints, n) <- introMotive motiveType motiveType (trail targets) B0 0
>     elimTrace $ "constraints: " ++ show constraints

Simplify the constraints to produce an updated list of binders and goal
type:
...
Here we go. First, we need to retrieve some information about our goal
and its context, before we start modifying the development.
...
We solve the problem by applying the eliminator. Since the binders
already contain the information we need in their second components, it
is straightforward to build the term we want and to give it. Note that
we have to look up the latest version of the rebuilt eliminator because
its definition will have been updated when the motive was defined.

>     Just (elim :=>: _) <- lookupName elimName
>     tt <- give $ N $ elim $## map snd binders

Now we have to move the methods. We use the usual trick: make new
definitions and solve the old goals with the new ones. First we collect
the types of the methods, quoting them (to expand the definition of the
...
This leaves us on the same goal we started with. For interactive use, we
will typically want to move to the first (lifted) method:
