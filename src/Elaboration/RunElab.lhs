...
It will return the term produced by elaboration, and a flag indicating
whether elaboration was completely successful or had to suspend. If
elaboration suspended when working on the curent goal, the term will be
a reference to that goal, so it cannot be solved.
...
As often, we are giving here a read-only access to the name supply.
Therefore, we must be careful not to let some generated names dangling
into some definitions, or clashes could happen.
...
If we are looking for a labelled type (e.g. to make a recursive call),
we search the hypotheses for a value with the same label.
...
On our way to the label, we instantiate the hypotheses with fresh
references.
...
value, i.e. it is unconstrained by the matching problem, we hope for a
solution.
...
If we are hoping for a proof of a proposition, we first try simplifying
it using the propositional simplification machinery.
...
and use reflexivity. This case arises frequently when proving label
equality to make recursive calls.
...
creating a new subgoal. Ideally we should cry rather than hoping for
something patently absurd: at the moment we reject some impossible hopes
